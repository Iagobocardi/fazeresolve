...
// Integrações de pagamento e mensageria não são necessárias para a demonstração
// local. Em produção, habilite-as explicitamente com ENABLE_JOBS=true.
if (process.env.ENABLE_JOBS === 'true') {
    require('./src/jobs/lembretes.job');
    require('./src/jobs/billing.job.js');
    require('./src/jobs/gracePeriod.job.js');
    require('./src/jobs/whatsappSender.job.js');
    require('./src/jobs/dunning.job.js');
    require('./src/jobs/paymentStatus.job.js');
    const pendingPaymentNotifierJob = require('./src/jobs/pendingPaymentNotifier.js');
    pendingPaymentNotifierJob.start();
    console.log('-> Jobs agendados foram habilitados.');
} else {
    console.log('-> Jobs externos desabilitados (defina ENABLE_JOBS=true para habilitá-los).');
}
...
const startServer = async () => {
    try {
        ...
        if (process.env.SEED_DEMO_DATA === 'true') {
            await require('./scripts/seed-demo').seedDemoData();
        }
